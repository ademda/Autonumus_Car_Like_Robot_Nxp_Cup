...
void ComputeAngleBasedSpeed() {
  ...
  // Above GAIN_THRESHOLD → linearly interpolate down to SPEED_MIN_TURN_MM_S at 90°.
  float desired_speed;
  if (angle_error_deg <= GAIN_THRESHOLD) {
    desired_speed = SPEED_MAX_MM_S;
  } else {
    // float t = (angle_error_deg - GAIN_THRESHOLD) / (90.0f - GAIN_THRESHOLD);
    // t = constrain(t, 0.0f, 1.0f);
    // desired_speed = SPEED_MAX_MM_S - t * (SPEED_MAX_MM_S - SPEED_MIN_TURN_MM_S);
    desired_speed = SPEED_MIN_TURN_MM_S;
  }

  // Respect strategy speed cap
  //desired_speed = min(desired_speed, (float)strategy_speed);
